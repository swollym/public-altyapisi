const {  kapsentcik, roleBackup, channelBackup } = require("../../../../Helpers/Schemas")
...
class Gmenu extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        const menu = await message.channel.send({ embeds: [embed.setDescription(`**Aşağıdaki Butonlardan Sunucunun Rol Ve Kanal Yedeğini Alıp Kurabilirsiniz!**`)], components: [row] })
...
async function gMenuRolKur() {
    ...
    deletedRoles.forEach(async (deletedRole) => {
        ...
        let kapsent = await kapsentcik.findOne({ guildID: config.guildID })
        roleBackup.findOne({ roleID: deletedRole.roleID }, async (err, data) => {
            ...
            for (let index = 0; index < Bots.length; index++) {
                ...
                if (members.length <= 0) {
                    ...
                    if (kapsent.guardLog) client.channels.cache.get(kapsent.guardLog).send(`[${deletedRole.roleID}] rolünün datasında üye verisi bulunamadığı veya üyelere zaten dağıtıldığı için dağıtım işlemi yapılamadı!`)
                    break;
                }
