...
class GuildInfo extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        let yetkili = message.guild.members.cache.filter(yetkili => yetkili.roles.cache.has(kapsent?.registerHammer)).size
...
${yonetici ? yonetici.map(kapsentciks => `${kapsentciks}`).join(' , ') : "Rolde bulunan kimse yok!"}
**────────────────────────**
\`\`\`cs
[URL/SUNUCU YÖNET] yetkisine sahip olan (${urlsunucu.length}) kişilerin listesi;\`\`\`
${urlsunucu ? urlsunucu.map(kapsentciks => `${kapsentciks}`).join(' , ') : "Rolde bulunan kimse yok!"}
**────────────────────────**
\`\`\`cs
[ROL YÖNET] yetkisine sahip olan (${rolyonet.length}) kişilerin listesi;\`\`\`
${rolyonet ? rolyonet.map(kapsentciks => `${kapsentciks}`).join(' , ') : "Rolde bulunan kimse yok!"}
**────────────────────────**
\`\`\`cs
[KANAL YÖNET] yetkisine sahip olan (${kanalyonet.length}) kişilerin listesi;\`\`\`
${kanalyonet ? kanalyonet.map(kapsentciks => `${kapsentciks}`).join(' , ') : "Rolde bulunan kimse yok!"}
**────────────────────────**
                `)],
                        ephemeral: true
                    })
