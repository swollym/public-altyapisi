...
class Unregister extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (member.roles.cache.has(kapsent.unregisterRole)) return message.channel.send({ embeds: [embed.setDescription(`**UYARI :** Bu kullanıcı zaten kayıtsız!`)]}).sil(10)
        ...
        member.setRoles(kapsent.unregisterRole)
        member.setNickname(`${kapsent.isimsembol} Kayıtsız`)
        ...
        client.channels.cache.get(kapsent.denetimLog).send({ embeds: [embed.setDescription(`${member} (\`${member.user.tag} - ${member.id}\`) kişisi ${message.author} tarafından \`${moment(Date.now()).format("LLL")}\` tarihinde kayıtsıza atıldı! ${emojis.onay}`)]})
