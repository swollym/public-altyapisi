...
class Woman extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (!member) return message.channel.send(`Kayıt yapabilmem için bir kullanıcı belirtmelisin **Örnek :** \`.e @kapsent/ID İsim | Yaş\``)
        if (member.roles.cache.has(kapsent.cezalıRol)) return message.channel.send(`Bu üye cezalı olduğu için işlemi gerçekleştiremedim!`).sil(10)
        ...
        await member.setNickname(`${setName}`, `Kadın Kayıt, Yetkili: ${message.author.id}`); await member.roles.remove([kapsent.unregisterRole]); await member.roles.add(kapsent.womanRoles)
        ...
        await Users.findOneAndUpdate({ userID: message.author.id }, { $inc: { TeyitNo: 1 } }, { upsert: true }).exec(); await Users.findOneAndUpdate({ userID: message.author.id }, { $push: { Teyitler: { userID: member.id, rol: kapsent.womanRoles[0], date: Date.now(), Gender: "Kadın" } } }, { upsert: true }); await Users.findOneAndUpdate({ userID: member.id }, { $push: { Names: { userID: message.author.id, Name: `${setName}`, rol: kapsent.womanRoles[0], islem: "Kayıt" } } }, { upsert: true }); await Users.findOneAndUpdate({ userID: member.id }, { $set: { Teyitci: { userID: message.author.id, Cinsiyet: kapsent.womanRoles[0], date: Date.now() } } }, { upsert: true }); if (kapsent.coinSystem === true) await coin.findOneAndUpdate({ guildID: member.guild.id, userID: author.id }, { $inc: { coin: config.registerCoin } }, { upsert: true }); message.member.updateTask(message.guild.id, "kayıt", 1, message.channel);
