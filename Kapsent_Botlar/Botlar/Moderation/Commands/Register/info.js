...
class KInfo extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        let teyitler = await message.channel.send({ embeds: [embed.setAuthor(member.user.tag, member.user.avatarURL()).setDescription(`${member} - Sunucuya ait olan Teyit verilerin :\n\n__Toplam Kayıt Sayın :__ **${data.TeyitNo ? data.TeyitNo : 0}** | (**${data.Teyitler ? data.Teyitler.filter(v => v.Gender === "Erkek").length : 0}** Erkek, **${data.Teyitler ? data.Teyitler.filter(v => v.Gender === "Kadın").length : 0}** Kadın)`).setColor('RANDOM').setTimestamp()], components: [row] })
        ...
        collector.on('collect', async (button, user) => {
            if (button.customId === "teyitler") {
                ...
                if (pages.length === 1) {
                    ...
                    const row = new Discord.MessageActionRow().addComponents(new Discord.MessageButton().setCustomId('tgeri').setLabel("◀").setStyle('PRIMARY'), new Discord.MessageButton().setCustomId('teyitiptal').setLabel("İptal").setStyle('DANGER'), new Discord.MessageButton().setCustomId('tileri').setLabel("▶").setStyle('PRIMARY'),);
