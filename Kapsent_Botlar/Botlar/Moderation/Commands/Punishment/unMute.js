...
class UnMute extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (!member.roles.cache.has(kapsent.mutedRole)) return message.reply({ embeds: [embed.setDescription(`**UYARI :** Bu kullanıcı zaten susturulmamış!`)]}).sil(10)
        ...
        await member.roles.remove(kapsent.mutedRole, `Susturma Kaldırma, Yetkili: ${message.author.id}`)
