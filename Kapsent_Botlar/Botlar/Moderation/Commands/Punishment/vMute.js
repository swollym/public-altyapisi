...
class Vmute extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (member.roles.cache.has(kapsent.vmutedRole)) return message.channel.send({ embeds: [embed.setDescription(`**UYARI :** BU kullanıcı zaten seste susturulmuş!`)]}).sil(10)
        ...
        await member.roles.add(kapsent.vmutedRole, `Ses Susturma, Yetkili: ${message.author.id}`)
