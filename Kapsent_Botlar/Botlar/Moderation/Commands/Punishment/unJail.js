...
class UnJail extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (!member.roles.cache.has(kapsent.jailedRole)) return message.reply({ embeds: [embed.setDescription(`**UYARI :** Bu kullanıcı zaten cezalıda değil!`)]}).sil(10)
        ...
        await member.setRoles(kapsent.unregisterRole, `Cezalı Kaldırma, Yetkili: ${message.author.id}`)
