const { kapsentcik, Penalties } = require('../../../../Helpers/Schemas')
...
class UnVMute extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (!member.roles.cache.has(kapsent.vmutedRole)) return message.reply({ embeds: [embed.setDescription(`**UYARI :** Bu kullanıcı zaten susturulmamış!`)]}).sil(10)
        ...
        await member.roles.remove(kapsent.vmutedRole, `Susturma Kaldırma, Yetkili: ${message.author.id}`)
        ...
        if (kapsent.muteLog) client.channels.cache.get(kapsent.muteLog).send({ embeds: [embed.setThumbnail(member.avatarURL({ dynamic: true })).setAuthor({ name: member.user.username, iconURL: member.user.avatarURL({ dynamic: true, size: 2048 })} ).setDescription(`${emojis.nokta} **Susturması Kaldırılan Kullanıcı :** ${member} (\`${member.user.username.replace(/\`/g, "")} - ${member.id}\`)\n${emojis.nokta} **Susturmayı Kaldıran Yetkili :** ${message.author} (\`${message.author.tag} - ${message.author.id}\`)\n${emojis.nokta} **Susturma Kaldırılma Zamanı :** \`${moment(Date.now()).format("LLL")}\``).setTitle(`Ses Susturma Kaldırma İşlemi!`).setFooter(`Ceza ID : #${ceza.id}`)] })
    }
}
