...
class Taggeds extends Command {
    ...
    async run(client, message, args, embed) {
        ...
        if (!message.member.roles.cache.has(kapsent.registerHammer)) return message.reply(`Bu komutu kullanmak için yeterli yetkiniz yok!`).sil(20)
        ...
        if (pages.length === 1) {
            await message.reply({ embeds: [embed.setDescription(`${member} adlı kullanıcının toplam **${sayi - 1}** taglısı bulundu!\n\n${pages[currentPage - 1].map(e => e ? ` <@!${e.userID}> - ${new Date(e.date).toTurkishFormatDate()}` : "").join("\n")}`).setFooter(`Developed By kapsent.`)] })
        } else {
            const row = new Discord.MessageActionRow().addComponents(new Discord.MessageButton().setCustomId('geri').setLabel("◀").setStyle('PRIMARY'), new Discord.MessageButton().setCustomId('iptal').setLabel("İptal").setStyle('DANGER'), new Discord.MessageButton().setCustomId('ileri').setLabel("▶").setStyle('PRIMARY'),);
