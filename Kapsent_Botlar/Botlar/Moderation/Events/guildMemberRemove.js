...
class GuildMemberRemove {
    ...
    async run(member) {
        ...
        if (!channel) return;
        if (member.user.bot) return channel.send({ content: `${emojis.left} **${member.user.tag}** sunucumuzdan ayrıldı! Davet eden: **Davetçi bulunamadı**` })
        const inviteMemberData = await Users.findOne({ userID: member.user.id }) || [];
        if (!inviteMemberData.Inviter) {
            return channel.send({ content: `${emojis.left} **${member.user.tag}** sunucumuzdan ayrıldı! Davet eden: **Davetçi bulunamadı**` });
            ...
            return channel.send({ content: `${emojis.left} **${member.user.tag}** sunucumuzdan ayrıldı! Davet eden: \`Sunucu Özel URL\` (**${total}** davet)` });
        } else {
            if (Date.now() - member.user.createdTimestamp <= config.userTime) {
                ...
                const total = inviterData ? inviterData.total : 0;
                return channel.send({ content: `${emojis.left} **${member.user.tag}** sunucumuzdan ayrıldı! Davet eden: **${inviter.tag}** (**${total}** davet)` })
            } else {
                const inviter = await client.users.fetch(inviteMemberData.Inviter.inviter);
