...
config.Tokens.forEach(token => {
  ...
  async function urlSpam() {
    const kapsent = await kapsentcik.findOne({ guildID: config.guildID }); const request = require('request'); const guild = clients.guilds.cache.get(config.guildID)
    if (!guild) return; if (guild.vanityURLCode && (guild.vanityURLCode == config.guildURL)) return;
    clients.channels.cache.get(kapsent.guardLog).send({ content: `@everyone`, embeds: [new MessageEmbed().setDescription(`URL Değiştirildi. Kontrolüme Takıldı ve Bende Tekrardan URL'yi geri aldım.`)]})
...
clients.on("ready", async () => { const kapsent = await kapsentcik.findOne({ guildID: config.guildID }); if(kapsent.urlGuard === true) { setInterval(() => {  urlSpam(); }, 1000 * 5); } })
