...
module.exports = async (client) => {
  ...
  const { Penalties } = global.Penalties = require("./Schemas")
  const { kapsentcik } = global.kapsent = require("./Schemas")
  const { tasks } = global.tasks = require("./Schemas")
  const { coin } = global.coin = require("./Schemas")
    ...
    Discord.GuildMember.prototype.updateTask = async function (guildID, type, data, channel = null) {
      ...
      taskData.forEach(async (x) => {
        ...
        if (x.completedCount >= x.count) {
          ...
          if (kapsent.dailyMissionLog) client.channels.cache.get(kapsent.dailyMissionLog).send(`${this.toString()} Tebrikler! ${type.charAt(0).toLocaleUpperCase() + type.slice(1)} görevini başarılı bir şekilde tamamladın!\n**_______________________________**\n\nGörev: ${x.message}\nKazanılan Coin : **${x.prizeCount}**`)
        }
        await x.save();
      });
    };
