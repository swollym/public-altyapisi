const { roleBackup, kapsentcik, channelBackup } = require("../Helpers/Schemas");
class Backup {
    static async RoleBackup() {
        ...
        if (guild) {
            ...
            if (kapsent.guardLog) client.channels.cache.get(kapsent.guardLog).send(`${emojis.onay} Güvenlik amaçlı rol verileri yedeklendi!`)
        };
    }
    static async channelBackup() {
        ...
        if (guild) {
            ...
            if (kapsent.guardLog) client.channels.cache.get(kapsent.guardLog).send(`${emojis.onay} Güvenlik amaçlı kanal verileri yedeklendi!`)
        }
    }
}
